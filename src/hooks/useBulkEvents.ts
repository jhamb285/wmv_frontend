...
export function useBulkEvents(options: UseBulkEventsOptions = {}) {
  const [eventsByVenue, setEventsByVenue] = useState<Record<number, Event[]>>({});
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const { venues = [], enabled = true } = options;

  // Create cache key for the current request (using venue_ids for consistency)
  const cacheKey = JSON.stringify({
    venue_ids: venues.map(v => Number(v.venue_id)).sort(),
  ...
  const fetchBulkEvents = useCallback(async () => {
    ...
    try {
      ...
      // Extract venue_ids for API call and caching
      const venue_ids = venues.map(venue => Number(venue.venue_id));
      const now = Date.now();

      console.log(`🚀 BULK EVENTS - Processing ${venues.length} venues by venue_id (same as map view)`);

      // Check cache for each venue (cache by venue_id)
      const cachedData: Record<number, Event[]> = {};
      const venueIdsToFetch: number[] = [];

      venues.forEach((venue) => {
        const venue_id = Number(venue.venue_id);
        const venueCacheKey = `${venue_id}:${cacheKey}`;
        const cachedEntry = bulkEventsCache.get(venueCacheKey);

        if (cachedEntry && now < cachedEntry.expiresAt) {
          console.log(`🚀 CACHE HIT for venue_id: ${venue_id}`);
          cachedData[venue_id] = cachedEntry.data;
        } else {
          console.log(`📡 CACHE MISS for venue_id: ${venue_id}`);
          venueIdsToFetch.push(venue_id);
          // Clean expired cache
          if (cachedEntry) {
      ...
      console.log(`📡 FETCHING ${venueIdsToFetch.length} venues from API by venue_id:`, venueIdsToFetch);

      // Fetch missing venues in bulk (using venue_ids only)
      ...
      if (result.success && result.data) {
        ...
        // Store fetched data in cache (keyed by venue_id)
        Object.keys(fetchedData).forEach(venue_id_str => {
          const venue_id = Number(venue_id_str);
          const venueCacheKey = `${venue_id}:${cacheKey}`;
          bulkEventsCache.set(venueCacheKey, {
            data: fetchedData[venue_id] || [],
            timestamp: now,
  ...
  // Helper function to get events for a specific venue (by venue_id)
  const getEventsForVenue = useCallback((venueId: number): Event[] => {
