...
interface CategoryPillsProps {
  ...
  venues: Venue[];
  inlineMode?: boolean; // When true, renders without fixed positioning (for TopNav integration)
}
...
const CategoryPills: React.FC<CategoryPillsProps> = ({
      ...
      <button
        ...
        style={{
          ...
          background: isSelected
            ? `${hexColor}CC` // Light fill with 80% opacity
            : `${hexColor}80`, // Light fill with 50% opacity
          border: 'none'
        }}
      >
        {displayName} ({count})
    ...
    return (
      <button
        ...
        style={{
          ...
          background: isSelected
            ? `${hexColor}CC` // Light fill with 80% opacity
            : `${hexColor}66`, // Light fill with 40% opacity (slightly lighter than primary)
          border: 'none'
        }}
      >
        {secondary}
  ...
  // Pills content (reusable for both modes)
  const pillsContent = (
    <>
  ...
  // Inline mode: just return the pills content (for TopNav integration)
  if (inlineMode) {
    return pillsContent;
  }

  // Default mode: return with fixed positioning and dark overlay
  return (
    <>
